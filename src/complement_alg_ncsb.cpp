...
public: // METHODS
  ...
  virtual bool subsum_less(const mstate& rhs, const std::set<unsigned>&  glob_reached) override {
      ...
      auto C_subs = std::includes(check_.begin(), check_.end(), rhs_ncsb->check_.begin(), rhs_ncsb->check_.end());
      auto S_subs = std::includes(safe_.begin(), safe_.end(), rhs_ncsb->safe_.begin(), rhs_ncsb->safe_.end());
      auto B_subs = std::includes(breakpoint_.begin(), breakpoint_.end(), rhs_ncsb->breakpoint_.begin(), rhs_ncsb->breakpoint_.end());

      return (C_subs && S_subs && B_subs);
  };
